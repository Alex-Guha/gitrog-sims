...
def handle_dredge(lib, trigs, landsInLib, libSize):
    found_loam = False
...
def sim(landsInLib, libSize, trigs):
    succ = 0
...
def ggt(minLands, maxLands, minTriggers, maxTriggers, libSize):
    result = []

    for i in tqdm.tqdm(range(minLands, maxLands)):
        result.append([str(i) + ":  "])  # Used to display the "numLands: "
        ...
        for j in range(minTriggers, maxTriggers):
            result[i - minLands].append(sim(i, libSize, j))
